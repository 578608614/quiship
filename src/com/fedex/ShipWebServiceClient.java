...
//	    requestedPackageLineItem.setSpecialServicesRequested(addPackageSpecialServicesRequested());
	...
	private static void writeServiceOutput(ProcessShipmentReply reply) throws Exception
	{
		...
		{	//==========================
			...
			System.out.println("交易id:"+trdetail.getCustomerTransactionId());
			//==========================
			//获取发货详情
			CompletedShipmentDetail shipInfo = reply.getCompletedShipmentDetail(); 
			//打印trackingNumber  测试为空
	...
	private static void printPackageRating(PackageRating packageRating){
		if(packageRating!=null){
			...
			for(int j=0; j < prd.length; j++)
			{
				...
				printMoney(prd[j].getBaseCharge(), "Base Charge", "    ");//基础费用
				printMoney(prd[j].getNetCharge(), "Net Charge", "    ");//网络费用
				printMoney(prd[j].getTotalFreightDiscounts(), "Total Freight Discounts", "    ");//总运费折扣
				printMoney(prd[j].getTotalRebates(), "Total Rebates", "    ");//总折扣
				printMoney(prd[j].getTotalSurcharges(), "Total Surcharge", "    ");//总附加费
